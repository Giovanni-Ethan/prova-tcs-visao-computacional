...
#--------------------------------------------#
# carregando a imagem e inspecionando ela
...
plt.imsave("binarizada.png", binaria, cmap='gray')
...
plt.title(f"Histograma da imagem (limiar do otsu = {limiar})")
plt.savefig("histograma.png")
#plt.show()
plt.close() #fecha a figura do histograma pra nao interferir nos proximos plt.imshow
...
plt.imsave("limpa.png", limpa, cmap='gray')
...
mascara_picos = np.zeros(distancia.shape, dtype=bool) #criamos uma tela em branco para marcarmos somente os pontos de pico
#o numpy espera que, pra marcar varios pontos de picos de uma vez, ele receba uma lista separada contendo somente os indices de linha e outra com os indices de coluna
mascara_picos[tuple(coords.T)] = True #fazemos a transposicao da matriz, e depois a convertemos tambem numa tupla de dois arrays, sendo um de linhas e um de colunas. Assim o numpy vai enxergar diversos pontos discretos de uma vez e marcamos como true todos esses pontos (naquela tela de antes)
...
plt.imsave("watershed.png", rotulos, cmap='nipy_spectral')
...
plt.imsave("escurecida_binarizada.png", binaria_escura, cmap='gray')
